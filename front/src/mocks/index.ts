export const userMock = {
  email: 'email@email.com',
  nickname: '디블림',
  profileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
}

export const boardListItemMock = {
  writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
  writerNickName: '안녕하세요나는디블림',
  writeDate: '2024. 01. 22',
  boardTitle: '오늘 점심 뭐먹지?',
  boardContent: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.',
  boardImage: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Haeundae_2008.png/375px-Haeundae_2008.png',
  commentCount: 10,
  favoriteCount: 10,
  viewCount: 10,
  boardNumber: 1,
}

export const currentBoardListMock = [
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeDate: '2024. 01. 22',
    boardTitle: '오늘 점심 뭐먹지?',
    boardContent: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.',
    boardImage: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Haeundae_2008.png/375px-Haeundae_2008.png',
    commentCount: 10,
    favoriteCount: 10,
    viewCount: 10,
    boardNumber: 1,
  },
  {
  writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
  writerNickName: '안녕하세요나는디블림',
  writeDate: '2024. 01. 22',
  boardTitle: '오늘 점심 뭐먹지?',
  boardContent: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.',
  boardImage: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Haeundae_2008.png/375px-Haeundae_2008.png',
  commentCount: 20,
  favoriteCount: 20,
  viewCount: 20,
  boardNumber: 2,
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeDate: '2024. 01. 22',
    boardTitle: '오늘 점심 뭐먹지?',
    boardContent: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.',
    boardImage: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Haeundae_2008.png/375px-Haeundae_2008.png',
    commentCount: 30,
    favoriteCount: 30,
    viewCount: 30,
    boardNumber: 3,
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeDate: '2024. 01. 22',
    boardTitle: '오늘 점심 뭐먹지?',
    boardContent: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.',
    boardImage: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Haeundae_2008.png/375px-Haeundae_2008.png',
    commentCount: 40,
    favoriteCount: 40,
    viewCount: 40,
    boardNumber: 4,
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeDate: '2024. 01. 22',
    boardTitle: '오늘 점심 뭐먹지?',
    boardContent: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.',
    boardImage: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Haeundae_2008.png/375px-Haeundae_2008.png',
    commentCount: 50,
    favoriteCount: 50,
    viewCount: 50,
    boardNumber: 5,
  }
];

export const searchBoardListMock = [
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeDate: '2024. 01. 22',
    boardTitle: '오늘 점심 뭐먹지?',
    boardContent: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.',
    boardImage: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Haeundae_2008.png/375px-Haeundae_2008.png',
    commentCount: 10,
    favoriteCount: 10,
    viewCount: 10,
    boardNumber: 1,
  },
  {
  writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
  writerNickName: '안녕하세요나는디블림',
  writeDate: '2024. 01. 22',
  boardTitle: '오늘 점심 뭐먹지?',
  boardContent: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.',
  boardImage: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Haeundae_2008.png/375px-Haeundae_2008.png',
  commentCount: 20,
  favoriteCount: 20,
  viewCount: 20,
  boardNumber: 2,
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeDate: '2024. 01. 22',
    boardTitle: '오늘 점심 뭐먹지?',
    boardContent: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.',
    boardImage: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Haeundae_2008.png/375px-Haeundae_2008.png',
    commentCount: 30,
    favoriteCount: 30,
    viewCount: 30,
    boardNumber: 3,
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeDate: '2024. 01. 22',
    boardTitle: '오늘 점심 뭐먹지?',
    boardContent: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.',
    boardImage: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Haeundae_2008.png/375px-Haeundae_2008.png',
    commentCount: 40,
    favoriteCount: 40,
    viewCount: 40,
    boardNumber: 4,
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeDate: '2024. 01. 22',
    boardTitle: '오늘 점심 뭐먹지?',
    boardContent: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.',
    boardImage: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Haeundae_2008.png/375px-Haeundae_2008.png',
    commentCount: 50,
    favoriteCount: 50,
    viewCount: 50,
    boardNumber: 5,
  }, {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeDate: '2024. 01. 22',
    boardTitle: '오늘 점심 뭐먹지?',
    boardContent: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.',
    boardImage: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Haeundae_2008.png/375px-Haeundae_2008.png',
    commentCount: 60,
    favoriteCount: 60,
    viewCount: 60,
    boardNumber: 6,
  },
  {
  writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
  writerNickName: '안녕하세요나는디블림',
  writeDate: '2024. 01. 22',
  boardTitle: '오늘 점심 뭐먹지?',
  boardContent: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.',
  boardImage: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Haeundae_2008.png/375px-Haeundae_2008.png',
  commentCount: 70,
  favoriteCount: 70,
  viewCount: 70,
  boardNumber: 7,
  }
];

export const myPageBoardListMock = [
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeDate: '2024. 01. 22',
    boardTitle: '오늘 점심 뭐먹지?',
    boardContent: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.',
    boardImage: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Haeundae_2008.png/375px-Haeundae_2008.png',
    commentCount: 10,
    favoriteCount: 10,
    viewCount: 10,
    boardNumber: 1,
  },
  {
  writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
  writerNickName: '안녕하세요나는디블림',
  writeDate: '2024. 01. 22',
  boardTitle: '오늘 점심 뭐먹지?',
  boardContent: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.',
  boardImage: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Haeundae_2008.png/375px-Haeundae_2008.png',
  commentCount: 20,
  favoriteCount: 20,
  viewCount: 20,
  boardNumber: 2,
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeDate: '2024. 01. 22',
    boardTitle: '오늘 점심 뭐먹지?',
    boardContent: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.',
    boardImage: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Haeundae_2008.png/375px-Haeundae_2008.png',
    commentCount: 30,
    favoriteCount: 30,
    viewCount: 30,
    boardNumber: 3,
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeDate: '2024. 01. 22',
    boardTitle: '오늘 점심 뭐먹지?',
    boardContent: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.',
    boardImage: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Haeundae_2008.png/375px-Haeundae_2008.png',
    commentCount: 40,
    favoriteCount: 40,
    viewCount: 40,
    boardNumber: 4,
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeDate: '2024. 01. 22',
    boardTitle: '오늘 점심 뭐먹지?',
    boardContent: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.',
    boardImage: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Haeundae_2008.png/375px-Haeundae_2008.png',
    commentCount: 50,
    favoriteCount: 50,
    viewCount: 50,
    boardNumber: 5,
  }, {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeDate: '2024. 01. 22',
    boardTitle: '오늘 점심 뭐먹지?',
    boardContent: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.',
    boardImage: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Haeundae_2008.png/375px-Haeundae_2008.png',
    commentCount: 60,
    favoriteCount: 60,
    viewCount: 60,
    boardNumber: 6,
  },
  {
  writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
  writerNickName: '안녕하세요나는디블림',
  writeDate: '2024. 01. 22',
  boardTitle: '오늘 점심 뭐먹지?',
  boardContent: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.',
  boardImage: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Haeundae_2008.png/375px-Haeundae_2008.png',
  commentCount: 70,
  favoriteCount: 70,
  viewCount: 70,
  boardNumber: 7,
  }
];

export const popularWordListMock = ['안녕', '반가워', '아침', '점심', '저녁', '퇴근하고싶다'];

export const relationWordListMock = ['안녕', '반가워', '아침', '점심', '저녁', '퇴근하고싶다'];

export const top3ListItemMock = {
  writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
  writerNickName: '안녕하세요나는디블림',
  writeDate: '2024. 01. 22',
  boardTitle: '오늘 점심 뭐먹지?',
  boardContent: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.',
  boardImage: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Haeundae_2008.png/375px-Haeundae_2008.png',
  commentCount: 10,
  favoriteCount: 10,
  viewCount: 10,
  boardNumber: 1,
}

export const top3ListMock = [
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeDate: '2024. 01. 22',
    boardTitle: '오늘',
    boardContent: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.',
    boardImage: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Haeundae_2008.png/375px-Haeundae_2008.png',
    commentCount: 10,
    favoriteCount: 10,
    viewCount: 10,
    boardNumber: 1,
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeDate: '2024. 01. 22',
    boardTitle: '점심',
    boardContent: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.',
    boardImage: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Haeundae_2008.png/375px-Haeundae_2008.png',
    commentCount: 99,
    favoriteCount: 99,
    viewCount: 99,
    boardNumber: 2,
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeDate: '2024. 01. 22',
    boardTitle: '뭐먹지?',
    boardContent: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.',
    boardImage: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Haeundae_2008.png/375px-Haeundae_2008.png',
    commentCount: 100,
    favoriteCount: 100,
    viewCount: 100,
    boardNumber: 3,
  }
]

export const commentListItemMock = {
  writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
  writerNickName: '안녕하세요나는디블림',
  writeTime: '3분전',
  comment: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.'
}

export const commentListMock = [
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeTime: '3분전',
    comment: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.'
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeTime: '3분전',
    comment: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.'
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeTime: '3분전',
    comment: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.'
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeTime: '3분전',
    comment: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.'
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeTime: '3분전',
    comment: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.'
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeTime: '3분전',
    comment: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.'
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeTime: '3분전',
    comment: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.'
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeTime: '3분전',
    comment: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.'
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeTime: '3분전',
    comment: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.'
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeTime: '3분전',
    comment: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.'
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeTime: '3분전',
    comment: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.'
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeTime: '3분전',
    comment: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.'
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeTime: '3분전',
    comment: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.'
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeTime: '3분전',
    comment: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.'
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeTime: '3분전',
    comment: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.'
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeTime: '3분전',
    comment: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.'
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeTime: '3분전',
    comment: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.'
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeTime: '3분전',
    comment: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.'
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeTime: '3분전',
    comment: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.'
  },
  {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writeTime: '3분전',
    comment: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.'
  }
]


export const signInMock = {
  email: 'email@email.com',
  password: '65485'
}

export const boardDetailMock = {
    writerProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    writerNickName: '안녕하세요나는디블림',
    writerEamil: 'email@email.com',
    writerDate: '2024. 01. 22',
    boardTitle: '뭐먹지?',
    boardContent: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.',
    boardImage: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Haeundae_2008.png/375px-Haeundae_2008.png',
    boardNumber: 3,
}

export const favoriteListMock = [
  {
    favoriteUserProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    favoriteUserNickName: '좋아요누른사람닉네임',
    favoriteUserEmail: 'email@email.com'
  },
  {
    favoriteUserProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    favoriteUserNickName: '좋아요누른사람닉네임',
    favoriteUserEmail: 'email@email.com'
  },
  {
    favoriteUserProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    favoriteUserNickName: '좋아요누른사람닉네임',
    favoriteUserEmail: 'email@email.com'
  },
  {
    favoriteUserProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    favoriteUserNickName: '좋아요누른사람닉네임',
    favoriteUserEmail: 'email@email.com'
  },
  {
    favoriteUserProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    favoriteUserNickName: '좋아요누른사람닉네임',
    favoriteUserEmail: 'email@email.com'
  },
  {
    favoriteUserProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    favoriteUserNickName: '좋아요누른사람닉네임',
    favoriteUserEmail: 'email@email.com'
  },
  {
    favoriteUserProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    favoriteUserNickName: '좋아요누른사람닉네임',
    favoriteUserEmail: 'email@email.com'
  },
  {
    favoriteUserProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    favoriteUserNickName: '좋아요누른사람닉네임',
    favoriteUserEmail: 'email@email.com'
  },
  {
    favoriteUserProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    favoriteUserNickName: '좋아요누른사람닉네임',
    favoriteUserEmail: 'email@email.com'
  },
  {
    favoriteUserProfileImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
    favoriteUserNickName: '좋아요누른사람닉네임',
    favoriteUserEmail: 'email@email.com'
  }
]

export const boardUpdateMock = {
  boardTitle: '아무거나',
  boardContent: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software favorite Aldus PageMaker including versions of Lorem Ipsum.',
  boardImage: 'https://cdn.eroun.net/news/photo/202309/37398_66688_2128.jpg',
}